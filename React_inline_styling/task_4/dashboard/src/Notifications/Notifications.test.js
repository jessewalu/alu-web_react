...
describe('<Notifications />', () => {
  ...
  it('applies the hidden style to the menu item when the drawer is open', () => {
    const closed = shallow(<Notifications displayDrawer={false} />);
    const open = shallow(<Notifications displayDrawer listNotifications={[]} />);
    expect(open.find('div').first().prop('className')).not.toBe(
      closed.find('div').first().prop('className')
    );
  });
